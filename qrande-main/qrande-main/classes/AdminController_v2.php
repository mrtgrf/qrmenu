...
class AdminController extends Controller {
    ...
    public function completeWaiterCall($id)
    {
        ...
        $this->db->update("waiter_calls", [
            ...
            "acknowledged_by" => $this->currentUser['id'],
    ...
    public function viewOrder($id)
    {
        ...
        $order = $this->db->fetchOne(
            "SELECT o.*, t.table_number, cs.customer_name, cs.customer_phone 
             FROM orders o 
    ...
    public function deleteCategory($id)
    {
        ...
        $itemCount = $this->db->fetchOne(
            "SELECT COUNT(*) as count FROM menu_items WHERE category_id = ? AND license_id = ?", 
            [$id, $this->currentLicense["id"]]
        )['count'];

        if ($itemCount > 0) {
            redirect("admin/menu?error=category_has_items");
            return;
        }
    ...
    public function addMenuItem()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if (empty($name) || empty($price) || empty($categoryId)) {
                $categories = $this->db->fetchAll("SELECT * FROM categories WHERE license_id = ? ORDER BY display_order ASC", [$this->currentLicense["id"]]);
                $this->view("admin/menu_item_form", ["categories" => $categories, "error" => "Gerekli alanları doldurun."]);
                return;
            }
    ...
    public function editMenuItem($id)
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if (empty($name) || empty($price) || empty($categoryId)) {
                $categories = $this->db->fetchAll("SELECT * FROM categories WHERE license_id = ? ORDER BY display_order ASC", [$this->currentLicense["id"]]);
                $this->view("admin/menu_item_form", ["categories" => $categories, "menuItem" => $menuItem, "error" => "Gerekli alanları doldurun."]);
                return;
            }
        ...
        $categories = $this->db->fetchAll("SELECT * FROM categories WHERE license_id = ? ORDER BY display_order ASC", [$this->currentLicense["id"]]);
        $this->view("admin/menu_item_form", ["categories" => $categories, "menuItem" => $menuItem]);
    }
    ...
    public function addTable()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            $existingTable = $this->db->fetchOne(
                "SELECT id FROM tables WHERE table_number = ? AND license_id = ?", 
                [$tableNumber, $this->currentLicense["id"]]
            );

            if ($existingTable) {
                $this->view("admin/table_form", ["error" => "Bu masa numarası zaten kullanılıyor."]);
                return;
            }
    ...
    public function editTable($id)
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            $existingTable = $this->db->fetchOne(
                "SELECT id FROM tables WHERE table_number = ? AND license_id = ? AND id != ?", 
                [$tableNumber, $this->currentLicense["id"], $id]
            );

            if ($existingTable) {
                $this->view("admin/table_form", ["table" => $table, "error" => "Bu masa numarası zaten kullanılıyor."]);
                return;
            }
    ...
    public function qrCode($id)
    {
        ...
        $this->view("admin/qr_code", ["table" => $table]);
    }
}
